...
<div class="container sidebar-layer bg-white mt-0 pb-5" id="sidebar-layer">
  ...
  <div class="col pb-4">
    {{-- data regency/provinci --}}
    @foreach ($regencies as $regency)
      <div id="layer_{{ $regency->regency_id }}" class="{{ $regency->regency === "Chachoengsao" ? "" : "d-none" }}">
                ...
                <div class="p-2">
                          ...
                          @foreach ($landUses as $landUse)
                            ...
                            @if ($landUse->lu_id == $luId && !in_array($landUse->landuse, $uniqueLandUses))
                              ...
                              <div class="form-check">
                                <input name="{{ $landUse->landuse }}" class="form-check-input border border-secondary"
                                  type="checkbox" value="" id="checkboxId_lu{{ $landUse->lu_id }}"
                                  data-layer="{{ $landUse->landuse }}">
                                <label class="form-check-label" for="{{ $landUse->landuse }}">
                                  {{ $landUse->landuse }}
                                </label>
                              </div>
                              @php
                  ...
                  @foreach ($spatials as $spatial)
                    @if ($spatial->group_id == $spatialGroup->group_id)
                      <div class="form-check">
                        <input name="{{ $spatial->name }}" class="form-check-input border border-secondary"
                          type="checkbox" value="" id="checkboxId{{ $spatial->sp_id }}"
                  ...
                  @foreach ($agricultures as $agriculture)
                    @if ($agriculture->group_id == $spatialGroup->group_id)
                      ...
                      @if (!in_array($class, $displayedClasses))
                        <div class="form-check">
                          <input name="{{ $class }}" class="form-check-input border border-secondary"
                            type="checkbox" value="" id="point_{{ $class }}"
                            data-layer="{{ $class }}">
        ...
        @if ($regency->regency == "Chachoengsao")
          <div class="border rounded mt-2">
            <!-- <div class="border-top"></div> -->
            <a class="text-decoration-none text-white" data-bs-toggle="collapse" href="#GEE_chachoengsao" role="button"
              aria-expanded="false" aria-controls="GEE_chachoengsao">
